...
function Classes() {
  ...
  // Class options for the dropdown filter
  const classOptions = useMemo(() => {
    if (!Array.isArray(groups) || groups.length === 0) return [];
    return groups
      .map((g) => {
        const classKey = g.class_pair || g.name;
    ...
    return groups.filter((g) => {
      ...
      const matchesClass =
        selectedClasses.length === 0
          ? true
          : selectedClasses.includes(g.class_pair || g.name);
      return matchesSearch && matchesClass;
    });
  }, [groups, search, selectedClasses]);
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
          ...
          <div className="w-full md:w-80">
            <input
              ...
              className="w-full rounded-[20px] border border-slate-200 px-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"
            />
          </div>
        </div>
